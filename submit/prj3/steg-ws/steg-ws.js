...
function createImage(app) {
  return async function(req, res) {
      ...
      const originalFileName = req.file.originalname;
         const [filename, type] = pathToNameExt(originalFileName);
      const nameimg = await app.locals.images.putBytes(group, new Uint8Array(req.file.buffer), type, );
          res.append('Location', requestUrl(req) + '/' + nameimg +"."+type);
...
function pathToNameExt(imgPath) {
  ...
  const name =imgPath.substr(0, typeDelimIndex);
  return [name, ext];
}
...
function stegHide(app) {
  return async function(req, res) {
   ...
   const bytes = await app.locals.images.get(group, name, "ppm");
     const ppm = new Ppm(name,new Uint8Array(bytes));
     const steg = new Steg(ppm);
     const bytesarray = await steg.hide(obj.msg);
     const status = await app.locals.images.putBytes(obj.outGroup,new Uint8Array(bytesarray.bytes),"ppm",);
     const httphead = "http://localhost";
      res.location(httphead+":"+app.locals.port+app.locals.base+"/"+STEG+"/outputs/"+status);
...
function stegUnhide(app) {
  return async function(req, res) {
    try {
     ...
     const ppm = new Ppm(name,new Uint8Array(bytes));
     const steg = new Steg(ppm);
     const msgstring = steg.unhide();
        const msgjson = {msg : msgstring};
      res.status(OK).send(msgjson);
    }
    catch (err) {
      const mapped = mapError(err);
      res.status(mapped.status).json(mapped);
